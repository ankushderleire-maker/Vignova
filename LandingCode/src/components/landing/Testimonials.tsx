                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {reviews.slice(0, 3).map((review, index) => (
                        <motion.div
                            ...
                            className="bg-slate-100/50 p-6 rounded-2xl border border-slate-200 relative"
                        >
                            ...
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-slate-900 font-bold text-sm">
                                    {review.name.charAt(0)}
                                </div>
                                <div>
                                    <h4 className="text-slate-900 font-medium text-sm">{review.name}</h4>
                                    <p className="text-slate-500 text-xs text-sm">{review.role}</p>
                                </div>
                            </div>
                        </motion.div>
                    ))}
