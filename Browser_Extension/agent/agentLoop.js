...
const AgentLoop = (() => {
    ...
    async function runIteration() {
        ...
        const run = runId;
        const active = () => isRunning && !isPaused && run === runId;
        const access = await chrome.runtime.sendMessage({ type: "CHECK_AUTOFILL_ACCESS" }).catch(() => null);
        if (!active()) return "done";
        if (!access?.success) {
            emitStatus("error", { message: access?.error || "Sign in with a paid plan to use Autofill." });
            return "error";
        }
        iteration++;
        ...
        // Native <select> fields need select_option, not fill_input
        const nativeSelectSelectors = new Set(
            pageState.fields.filter((f) => f.tag === "select").map((f) => f.selector)
        );
        for (const action of allLocalActions) {
            if (action.action !== "fill_input") continue;
            if (reactSelectSelectors.has(action.selector)) {
                action.action = "select_react";
            } else if (nativeSelectSelectors.has(action.selector)) {
        ...
        if (newLocalActions.length > 0) {
            ...
            const result = await Executor.executeAll(newLocalActions, 100, active);
        ...
        if (unmatched.length > 0) {
            ...
            if (reallyUnmatched.length > 0) {
                ...
                if (batchActions.length > 0) {
                    ...
                    const allowedActions = new Set(['fill_input', 'select_option', 'select_react', 'answer_question']);
                    const fieldSelectors = new Set(reallyUnmatched.map(field => field.selector));
                    const newAIActions = batchActions.filter(a => allowedActions.has(a.action) && fieldSelectors.has(a.selector) && !filledSelectors.has(a.selector));
                    if (newAIActions.length > 0) {
                        ...
                        const Executor = window.Vignova_Executor;
                        const result = await Executor.executeAll(newAIActions, 200, active);
        ...
        if (!active()) return "done";
        emitStatus('done', { message: 'Autofill finished. Review the fields and continue or submit the application yourself.' });
        return 'done';
    }
    ...
    async function start(onStatus) {
        ...
        while (isRunning && run === runId && iteration < MAX_ITERATIONS) {
            if (isPaused) {
