...
const PageObserver = (() => {
    ...
    function getLabel(el) {
        ...
        // Some form builders place "First Name" and "Last Name" under the
        // inputs as helper text. Use that before falling back to the parent
        // "Name" group label, otherwise the first field can be treated as a
        // full-name field and remain empty.
        const next = el.nextElementSibling;
        if (next && /^(LABEL|SPAN|DIV|P|SMALL)$/.test(next.tagName) && !next.querySelector("input, textarea, select")) {
    ...
    /**
     * Custom-styled radios/checkboxes often hide the real input (opacity 0,
     * offscreen) and show a styled label instead — treat those as usable.
     */
    ...
    /**
     * Label of one radio/checkbox option (its own text, not the question).
     */
    ...
    /**
     * The question text for a radio/checkbox group — fieldset legend or the
     * label of the surrounding field container.
     */
    function getGroupLabel(el) {
        ...
        const container = el.closest(
            '[role="radiogroup"], [role="group"], [class*="field"], [class*="question"], [class*="form-group"]'
        );
        if (container) {
            const lbl = container.querySelector("legend, [class*='label'], [class*='question']");
    ...
    function extractFields() {
        const fields = [];
        const radioGroups = new Map(); // group name → field entry
        ...
        document.querySelectorAll("input").forEach((el) => {
            ...
            if (!isVisible(el) && !(isChoice && hasVisibleLabel(el))) return;

            // Radios: one field per GROUP with its options, not one per button —
            // so the question is matched once and answered once
            if (type === "radio") {
                const groupKey = el.name || getSelector(el);
                const optionLabel = getOptionLabel(el);
                const existing = radioGroups.get(groupKey);
                if (existing) {
                    existing.options.push({ value: el.value, text: optionLabel, selected: el.checked });
                    if (el.checked) existing.currentValue = optionLabel || el.value;
                    return;
                }
                const group = {
                    tag: "radio-group",
                    ...
                    currentValue: el.checked ? (optionLabel || el.value) : "",
                    required: el.required || el.getAttribute("aria-required") === "true",
                    options: [{ value: el.value, text: optionLabel, selected: el.checked }],
                };
            ...
            fields.push({
                ...
                id: el.id || null,
                name: el.name || null,
                label: type === "checkbox" ? (getOptionLabel(el) || getLabel(el)) : getLabel(el),
                placeholder: el.placeholder || null,
                selector: getSelector(el),
                currentValue: type === "checkbox" ? (el.checked ? "checked" : "") : (el.value || ""),
                required: el.required || el.getAttribute("aria-required") === "true",
                options: null,
            });
        });
        ...
        document.querySelectorAll('[contenteditable="true"], [contenteditable=""]').forEach((el) => {
            ...
            // Skip nested editors — only the outermost is the field
            if (el.parentElement && el.parentElement.closest('[contenteditable="true"], [contenteditable=""]')) return;
            fields.push({
                tag: "contenteditable",
