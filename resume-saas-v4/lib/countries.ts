/**
 * ISO 3166-1 countries with their international dialling codes.
 *
 * One list, because there were two: the signup form had a country selector
 * that was a static flag with no dropdown behind it, and the post-OAuth
 * "complete your profile" step offered six countries, so anyone outside them
 * could not finish signing up at all.
 *
 * Codes are the alpha-2 values stored on `users.country`, which the register
 * route validates as exactly two characters.
 *
 * Flags are derived from the code rather than typed out — the regional
 * indicator symbols are just the letters offset into a separate block — so
...
/** Alphabetical by name, which is the order someone scanning a list expects. */
 ...
 *
 * Regional indicator symbols sit at U+1F1E6 ("A") through U+1F1FF ("Z"), so a
 * pair of them renders as that country's flag. Returns an empty string for
 * anything that isn't two letters, since a broken code should show nothing
 * rather than tofu.
 */
