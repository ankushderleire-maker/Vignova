        ...
        <motion.div key="t0" variants={container} initial="hidden" animate="show" exit="exit" className="absolute z-20 inset-0 p-6 md:p-10 flex flex-col items-center justify-start overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <div className="w-full max-w-4xl mt-2">
            <motion.div variants={item} className="flex items-center justify-between mb-8">
              ...
              <div className="px-5 py-3 rounded-xl bg-violet-600 text-white text-sm font-bold flex items-center gap-2 shadow-lg shadow-violet-600/20 relative">
            ...
            <div className="p-6 flex flex-col gap-4">
              <div>
                ...
                <div className="h-10 rounded-xl border border-gray-200 bg-gray-50 px-3 flex items-center text-sm font-medium text-gray-900">
                  <span className="border-r-2 border-violet-500 animate-[blink_1s_step-end_infinite]">{typed}</span>
                </div>
              </div>
              <div>
              ...
              <div className="mt-4 h-11 rounded-xl bg-violet-600 text-white text-sm font-bold flex items-center justify-center relative">
                Save Job
                {typed === "OpenAI" && (
                  <motion.div
            ...
            <div className="w-full flex-1 flex gap-4 min-h-0">
              {['Saved', 'Applied', 'Interviewing', 'Offer'].map((col, ci) => (
                <motion.div key={col} variants={item} className="flex-1 flex flex-col min-w-0">
                  <div className={`text-xs font-bold uppercase tracking-wider mb-3 ${ci===0?'text-gray-500':ci===1?'text-violet-600':ci===2?'text-amber-600':'text-violet-600'}`}>{col}</div>
                  <div className="bg-gray-50 rounded-2xl border border-gray-100 p-3 flex-1 space-y-3 overflow-y-auto shadow-inner [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {ci===0&&<>
                      <motion.div initial={{opacity:0,scale:0.9,y:10}} animate={{opacity:1,scale:1,y:0}} transition={{type:"spring"}} className="bg-white rounded-xl border border-violet-200 p-3.5 shadow-sm ring-2 ring-violet-100"><div className="text-sm font-bold text-gray-800 mb-1">Software Engineer</div><div className="text-xs text-gray-500 flex items-center gap-1.5"><Building2 className="w-3.5 h-3.5"/>OpenAI</div></motion.div>
                    ...
                    {ci===3&&<>
                      <div className="bg-white rounded-xl border border-violet-200 p-3.5 shadow-sm"><div className="text-sm font-bold text-gray-800 mb-1">Full Stack</div><div className="text-xs text-violet-600 font-bold bg-violet-50 px-2 py-1 rounded w-fit mt-1.5">🎉 Offer!</div></div>
                      <div className="bg-white rounded-xl border border-violet-200 p-3.5 shadow-sm"><div className="text-sm font-bold text-gray-800 mb-1">SWE L4</div><div className="text-xs text-gray-500 flex items-center gap-1.5 mb-1.5"><Building2 className="w-3.5 h-3.5"/>Google</div><div className="text-xs text-violet-600 font-bold bg-violet-50 px-2 py-1 rounded w-fit">🎉 Offer!</div></div>
                    </>}
                  </div>
                </motion.div>
              ))}
