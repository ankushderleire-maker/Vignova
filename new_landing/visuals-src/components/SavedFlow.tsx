          ...
          <motion.div variants={item} className="bg-white rounded-2xl border border-gray-200 shadow-xl overflow-hidden">
            ...
            {resumes.map((r, i) => (
              <motion.div key={i} variants={item} className="grid grid-cols-4 gap-4 px-6 py-4 border-b border-gray-50 items-center hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-2"><FileText className="w-4 h-4 text-violet-500" /><span className="text-sm font-medium text-gray-800 truncate">{r.name}</span></div>
                <span className="text-xs text-gray-400">{r.date}</span>
                <div className={`text-xs font-bold px-2 py-1 rounded w-fit ${r.score >= 95 ? 'bg-violet-50 text-violet-600' : r.score >= 90 ? 'bg-violet-50 text-violet-600' : 'bg-amber-50 text-amber-600'}`}>{r.score}%</div>
                <div className="flex items-center gap-2"><div className="w-8 h-8 rounded bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-violet-50 hover:text-violet-600 cursor-pointer transition-colors"><Download className="w-4 h-4" /></div><div className="w-8 h-8 rounded bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-violet-50 hover:text-violet-600 cursor-pointer transition-colors"><Eye className="w-4 h-4" /></div></div>
              </motion.div>
            ))}
          </motion.div>
        </div>
