        ...
        <motion.div key="j0" variants={container} initial="hidden" animate="show" exit="exit" className="absolute z-20 inset-0 p-6 md:p-10 flex flex-col items-center justify-start overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <div className="w-full max-w-3xl mt-2">
            ...
            <motion.div variants={item} className="bg-white rounded-2xl border border-gray-200 p-4 shadow-xl flex items-center gap-3">
              ...
              <span className="text-gray-800 text-lg font-medium border-r-2 border-violet-500 animate-[blink_1s_step-end_infinite]">{typed}</span>
            </motion.div>
          </div>
        </motion.div>
      )}
          ...
          <AnimatePresence>
            {step === 2 && (
              <motion.div initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white/90 backdrop-blur-md rounded-2xl shadow-[0_20px_40px_-10px_rgba(0,0,0,0.2)] border border-gray-100 p-5 flex items-center gap-4 w-max">
                <div className="w-10 h-10 bg-violet-100 text-violet-600 rounded-full flex items-center justify-center shrink-0"><CheckCircle2 className="w-6 h-6"/></div>
                <div>
            ...
            {jobs.map((job, i) => (
              <motion.div key={i} variants={item} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className={`text-sm font-bold px-3 py-1 rounded-lg ${job.match>=95?'bg-violet-50 text-violet-600 border border-violet-200':'bg-violet-50 text-violet-600 border border-violet-200'}`}>{job.match}% Match</div>
                </div>
                ...
                <div className="flex items-center gap-3 mt-4 pt-4 border-t border-gray-100">
                  ...
                  {i === 0 && step === 2 ? (
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex-1 h-11 rounded-xl bg-violet-50 border border-violet-200 text-violet-600 text-sm font-bold flex items-center justify-center gap-2">
                      <CheckCircle2 className="w-4 h-4" /> Applied
                    </motion.div>
                  ) : (
                    <div className="flex-1 h-11 rounded-xl bg-violet-600 text-white text-sm font-bold flex items-center justify-center relative hover:bg-violet-700 transition-colors cursor-pointer">
                      Quick Apply
